...
func (cli *DockerCli) CmdCp(args ...string) error {
	cmd := cli.Subcmd(
		"cp",
		"CONTAINER:PATH LOCALPATH|- or LOCALPATH|- or CONTAINER:PATH",
		strings.Join([]string{
...
func (cli *DockerCli) statContainerPath(containerName, path string) (types.ContainerPathStat, error) {
	...
	response, err := cli.callRaw("HEAD", urlStr, nil, nil)
	if err != nil {
		return stat, err
	}
...
func (cli *DockerCli) copyFromContainer(srcContainer, srcPath, dstPath string) (err error) {
	...
	response, err := cli.callRaw("GET", urlStr, nil, nil)
	if err != nil {
		return err
	}
	defer response.body.Close()
...
func (cli *DockerCli) copyToContainer(srcPath, dstContainer, dstPath string) (err error) {
	...
	response, err := cli.streamRaw("PUT", urlStr, &streamOpts{in: content})
	if err != nil {
		return err
	}
	defer response.body.Close()
